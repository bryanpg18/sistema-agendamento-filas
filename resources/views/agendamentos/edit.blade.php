...
@section('conteudo')
    @php $horarioAtual = old('horario', substr($agendamento->horario, 0, 5)); @endphp
    <div class="max-w-2xl mx-auto"
         x-data="{
            data: '{{ old('data', $agendamento->data->format('Y-m-d')) }}',
            horarios: [],
            carregando: false,
            async buscarHorarios() {
                ...
                try {
                    const resposta = await fetch(`{{ route('agendamentos.horarios-disponiveis') }}?data=${this.data}&agendamento_id={{ $agendamento->id }}`, {
        ...
        <div class="mb-6">
            <p class="text-sm text-slate-400 dark:text-slate-500">
                <a href="{{ route('agendamentos.index') }}" class="hover:underline">Agendamentos</a> &gt; Editar
            </p>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white mt-1">Editar Agendamento</h1>
        </div>
            ...
            <form method="POST" action="{{ route('agendamentos.update', $agendamento) }}" class="space-y-4">
                    ...
                    <div class="relative">
                        <select name="cliente_id"
                            ...
                            @foreach ($clientes as $cliente)
                                <option value="{{ $cliente->id }}" @selected(old('cliente_id', $agendamento->cliente_id) == $cliente->id)>
                    ...
                    <div class="relative">
                        <select name="servico_id"
                            ...
                            @foreach ($servicos as $servico)
                                <option value="{{ $servico->id }}" @selected(old('servico_id', $agendamento->servico_id) == $servico->id)>
                                    {{ $servico->nome }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="relative">
                            <select name="horario" :disabled="! data || carregando"
                                ...
                                <template x-for="hora in horarios" :key="hora">
                                    <option :value="hora" x-text="hora" :selected="hora === '{{ $horarioAtual }}'"></option>
                                </template>
                            </select>
                            <svg class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                ...
                <div>
                    ...
                    <textarea name="observacoes" rows="3"
                              class="w-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-600">{{ old('observacoes', $agendamento->observacoes) }}</textarea>
