            ...
            <div class="w-full max-w-4xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl shadow-xl overflow-hidden flex flex-col md:flex-row transition-colors">
                ...
                <div class="w-full md:w-1/2 p-8 md:p-10 flex flex-col justify-center">
                    <div class="flex items-center justify-between mb-1">
                        <h2 class="text-2xl font-bold text-slate-900 dark:text-white">Acesse sua conta</h2>
                        <!-- Theme Toggle Button -->
                        <button type="button"
                                x-data="{
                    ...
                    <p class="text-slate-500 dark:text-slate-400 mb-6 text-sm">Entre para continuar</p>
                    ...
                    <form method="POST" action="{{ route('login') }}" class="space-y-4">
                        ...
                        <div class="flex items-center">
                            <input id="remember_me" type="checkbox" name="remember"
                                class="rounded border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-teal-600 shadow-sm focus:ring-teal-600">
                            <label for="remember_me" class="ms-2 text-sm text-slate-600 dark:text-slate-400">Lembrar de mim</label>
                        </div>
                        ...
                        <div class="text-center mt-3 space-y-2">
                            @if (Route::has('password.request'))
                                <div>
                                    <a href="{{ route('password.request') }}" class="text-sm text-teal-700 dark:text-teal-400 hover:underline">
                                        Esqueceu sua senha?
                                    </a>
                                </div>
                            @endif
                            @if (Route::has('register'))
                                <div>
                                    ...
                                    <a href="{{ route('register') }}" class="text-sm text-teal-700 dark:text-teal-400 font-semibold hover:underline">
                                        Criar conta
                                    </a>
                                </div>
                            @endif
